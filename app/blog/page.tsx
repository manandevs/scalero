  ...
  return (
    <main className="bg-white min-h-screen py-12 lg:py-20 px-4 sm:px-6 font-bpmf">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10 lg:mb-16">
        <h1 className="text-4xl lg:text-5xl font-semibold mb-4">博客</h1>
        <p className="text-gray-500 text-sm sm:text-base">
          阅读我们的最新文章、深度洞察与应用增长技巧，助力业务爆发。
        </p>
      </div>

      {/* Blog Cards */}
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-4">
        {blogPosts.map((post, i) => (
          <div key={i}>
            ...
            <div className="p-2 flex flex-col gap-1.5 mt-2 lg:mt-0">
              <Link href={`/blog/${post.id}`}>
                <h2 className="text-lg lg:text-xl font-semibold hover:text-red-600 transition-colors">
                  {post.title}
                </h2>
              </Link>
              <p className="text-gray-500 text-sm sm:text-base">{post.excerpt}</p>

              <div className="text-gray-400 flex justify-between text-xs sm:text-sm mt-1">
              ...
              <div className="flex flex-col xl:flex-row gap-3 xl:gap-4 mt-4 lg:mt-4">
                <Link href={`/blog/${post.id}`} className="w-full xl:w-fit">
                  <button className="flex w-full h-fit items-center justify-center gap-2 rounded-full
                    ...
                    active:shadow-[inset_0px_-4px_4px_0px_#FF7961,0px_0px_0px_2px_#FFCDD2] font-bpmf text-sm lg:text-base">
                    阅读文章
                  </button>
                </Link>

                <Link href={"#"} className="w-full xl:w-fit">
                  <button className="flex w-full h-fit items-center justify-center gap-2 rounded-full
                    ...
                    transition-transform duration-250 hover:-translate-y-1 active:translate-y-1
                    active:shadow-[inset_0px_-4px_4px_0px_#CE93D8,0px_0px_0px_2px_#E1BEE7] font-bpmf text-sm lg:text-base">
                    查看网站
                  </button>
                </Link>
              </div>
